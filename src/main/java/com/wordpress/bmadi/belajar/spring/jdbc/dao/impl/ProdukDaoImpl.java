package com.wordpress.bmadi.belajar.spring.jdbc.dao.impl;
...
public class ProdukDaoImpl implements ProdukDao {
    ...
    public void simpan(Produk p) {
        if (p.getId() == null) {
            try {
                ...
                String queryInsert = "INSERT INTO produk (kode, nama, harga)VALUES (?, ?, ?)";
            ...
            try {
                Connection conn = dataSource.getConnection();
                String queryUpdate = "UPDATE produk SET kode=?, nama=?, harga=? WHERE Id=?";
    ...
    public void hapus(Produk p) {
        try {
            Connection conn = dataSource.getConnection();
            String queryDelete = "DELETE FROM produk WHERE id=?";
